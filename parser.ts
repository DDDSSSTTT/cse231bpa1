...
export function traverseArgs(c : TreeCursor,s : string){
...
export function traverseExpr(c : TreeCursor, s : string) : Expr {
  switch(c.type.name) {
    ...
    case "CallExpression":
      ...
      throw new Error("PARSE ERROR: function call with error arity")

    case "UnaryExpression":
      c.firstChild();
      const uop = s.substring(c.from, c.to);
      if (uop!== "-" && uop !== "+")
        throw new Error("PARE ERROR: unimplemented unary op");
      c.nextSibling();
      var num = Number(uop + s.substring(c.from, c.to));
      if (isNaN(num))
        throw new Error("PARSE ERROR: unary operation failed")
      c.parent();
      return { tag: "num", value: num }
    case "BinaryExpression":
      c.firstChild();//c for cursor, Into the Binary Expression
      ...
      switch(s.substring(c.from,c.to)){
        ...
        case "*":
          ...
          break;
        default:
          throw new Error("PARSE ERROR: unknown binary ops");
      }
      c.nextSibling();
